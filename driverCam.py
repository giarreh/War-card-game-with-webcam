...
def card_finder(img):
    ...
    for zoom in range(len(images_zoom)):
        ...
        height, width = thresh_card.shape[:2]
        c_height = height/2
        c_width = width/2
    ...
    if len(cards_array_rank) == len(cards_array_suit):
        for t in range(len(cards_array_rank)):
            if len(cards_array_rank[t]) != 0 and len(cards_array_rank[t]) != 1 and len(cards_array_suit[t]) != 0 and len(cards_array_suit[t]) != 1: #check
                ...
                #cv2.imshow("Scanning...", Rank_img[0])
                #cv2.waitKey(0)
...
def find_card_info(contours, card_height):
    ...
    for con1 in range(len(contours_centers)):
        current_dist1 = math.dist((card_height[0]/2, card_height[1]/2), (contours_centers[con][0], contours_centers[con][1]))
...
def warp_contour(image, pts, w, h):
    ...
    differance = np.diff(pts, axis=-1)
    top_right = pts[np.argmin(differance)]
    bottom_left = pts[np.argmax(differance)]
    ...
    if w >= 1.2 * h:  # If horizontally oriented
        ...
        temp_card[1] = top_left
        temp_card[2] = top_right
        temp_card[3] = bottom_right

    if w > 0.8 * h and w < 1.2 * h:
        if pts[1][0][1] <= pts[3][0][1]:
            temp_card[0] = pts[1][0]  # Top left
            temp_card[1] = pts[0][0]  # Top right
            temp_card[2] = pts[3][0]  # Bottom right
            temp_card[3] = pts[2][0]  # Bottom left

        if pts[1][0][1] > pts[3][0][1]:
            temp_card[0] = pts[0][0]  # Top left
            temp_card[1] = pts[3][0]  # Top right
            temp_card[2] = pts[2][0]  # Bottom right
            temp_card[3] = pts[1][0]  # Bottom left

    maxWidth = 200
    maxHeight = 300

    distance = np.array([[0, 0], [maxWidth - 1, 0], [maxWidth - 1, maxHeight - 1], [0, maxHeight - 1]], np.float32)
    M = cv2.getPerspectiveTransform(temp_card, distance)
...
def card_scanner(cam):
    ...
    while True:
        ...
        if i < 13:
            ...
            cv2.putText(frame, "a=next, d=last, q= quit", (100, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 3,
                        cv2.LINE_AA)
            cv2.putText(frame, "x=picture, w=save, s=Throw", (100, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 3,
                        cv2.LINE_AA)
        if i > 12 and n > -1:
